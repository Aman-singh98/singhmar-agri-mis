...
// //  *
// //  * Trigger points:
// //  *  1. Micada excel upload   — docsArray pass karo (sheet order preserved)
// //  *  2. MainFile create/update — sirf appIds pass karo (DB se latest lega)
// //  *
// //  * @param {string|string[]|object[]} input
// //  *   - string / string[]  → appId(s) — DB se last inserted record ka shortStatus lega
// //  *   - object[]           → parsed docs array from sheet — sheet ki last row ka shortStatus lega (recommended for upload)
// //  */
// // export const syncApplicationStatus = async (input) => {
// //   if (!input) return;

// //   let statusMap = {};

// //   // ── CASE 1: docs array from sheet (upload flow) ──────────────────────────
// //   // Sheet mein same appId ke multiple rows ho sakte hain
// //   // Last occurrence = sheet ka sabse neeche wala row = latest status
// //   if (Array.isArray(input) && input.length > 0 && typeof input[0] === "object" && input[0].appId !== undefined) {
// //     for (const doc of input) {
// //       if (!doc.appId) continue;
// //       // Har iteration mein overwrite — loop end pe last row ka value bachega
// //       statusMap[doc.appId] = doc.shortStatus ?? null;
// //     }
// //   }

// //   // ── CASE 2: appId string(s) — MainFile create/update flow ────────────────
// //   // DB se last inserted record ka shortStatus lo
// //   else {
...
//   { mainKey: "name",           micadaKey: "farmerName"    },
//   { mainKey: "irrigationType", micadaKey: "programmeName" },
//   { mainKey: "areaInAcre",    micadaKey: "totalArea"     },
// ];
...
//  *   - MICADA mein miNumber hai hi nahi            → error = null (untouched)
//  *
...
//     let errorVal = null;
//     if (micadaRow) {
//       const hasMismatch = COMPARE_FIELDS.some(
//         (f) => normalize(doc[f.mainKey]) !== normalize(micadaRow[f.micadaKey])
//       );
...
// export { syncApplicationStatus, syncMismatchErrors };
