...
//    { label: "Dealer Name", w: 90,  align: "left"   },
//    { label: "Date",        w: 55,  align: "center" },
//    { label: "Amount",      w: 55,  align: "right"  },
//    { label: "Remark1",     w: 100, align: "left"   },
//    { label: "Remark2",     w: 100, align: "left"   },
// ];
...
//    { label: "Remark1", w: 120, align: "left"   },
//    { label: "Remark2", w: 120, align: "left"   },
// ];
...
//    cols[last - 1].w += Math.floor(leftover / 2);
//    cols[last].w     += leftover - Math.floor(leftover / 2);
//    return cols;
// }
...
//       .text("Material Dispatch — Receipts", MARGIN, 10, { lineBreak: false });
...
// // ── Main export ───────────────────────────────────────────────────────────────
// export async function generateMaterialDispatchReceiptsPDF(records, res, { financialYear, dealerName, dealerCode } = {}) {
//    try {
...
//          `attachment; filename="material-dispatch-receipts-${fileDate()}.pdf"`);
//       doc.pipe(res);
...
//       // withDealer = true when showing all dealers (no filter), false when filtered to one dealer
...
//    } catch (err) {
//       console.error("Material Dispatch Receipts PDF error:", err);
//       if (!res.headersSent) {
//          res.status(500).json({ message: "Failed to generate Material Dispatch Receipts PDF" });
//       }
//    }
// }
...
export async function generateMaterialDispatchReceiptsPDF(records, res, { financialYear, dealerName, dealerCode } = {}) {
   try {
      ...
      const totals = { count: sortedRecords.length, amount: 0 };
