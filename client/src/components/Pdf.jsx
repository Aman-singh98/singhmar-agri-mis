...
function Pdf({
	...
	const handleClick = useCallback(async () => {
		...
		try {
			...
			const response = await api.post("/download/pdf", payload, { responseType: "blob" });
	...
	}, [ids, model, fileName, format, reportType, financialYear, dealerName, data]);
	...
	function Spinner() {
		return (
			<svg
				style={{ animation: "pdfSpin 0.75s linear infinite", flexShrink: 0 }}
	...
	if (activeVariant === "compact") {
		return (
          ...
          `}
				>
					{loading ? <Spinner /> : <FaFilePdf className="w-3.5 h-3.5" />}
				</button>
			</>
		);
	}
