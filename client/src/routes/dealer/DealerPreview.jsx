...
// 		setFarmerDealerCode(formatDealerCode(raw, farmerDealerCode));
// 		setCodeError("");
// 	};
...
// 		if (!DEALER_CODE_REGEX.test(farmerDealerCode.trim())) return setCodeError("Format must be XX/00 (e.g. MP/08)");

// 		const unchanged =
// 			dealerName.trim().toLowerCase() === dealer?.dealerName?.toLowerCase() &&
// 			farmerDealerCode.trim().toUpperCase() === dealer?.farmerDealerCode?.toUpperCase();
...
// 											placeholder="MP/08"
// 											maxLength={5}
// 											className={`w-full pl-9 pr-3 py-2.5 text-sm border  focus:outline-none focus:ring-2 font-mono uppercase transition
//                         ${codeError
// 													? "border-red-300 focus:ring-red-300 bg-red-50"
// 													: "border-gray-200 focus:ring-green-600/30 focus:border-green-500 bg-white"}`}
// 										/>
// 									</div>
// 									{codeError
// 										? <span className="text-xs text-red-500 flex items-center gap-1"><HiExclamation className="w-3 h-3" />{codeError}</span>
