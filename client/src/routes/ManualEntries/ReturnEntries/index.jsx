...
import ReturnEntryForm from "./components/ReturnEntryForm";
import ReturnEntryTable from "./components/ReturnEntryTable";
...
import FlashAlert from "../ShiftingEntries/components/FlashAlert";

// ─────────────────────────────────────────────────────────────────────────────
// ReturnEntries — layout identical to ShiftingEntrySection
//   • flex-col h-full, fixed header + tab bar, flex-1 overflow-y-auto body
//   • No inline CSS · no rounded-* · standard Tailwind only
// ─────────────────────────────────────────────────────────────────────────────

const PAGE_TABS = [
   { key: "form",    label: "New Return" },
...
function ReturnEntries() {
   ...
   const fetchEntries = useCallback(() => {
      setLoadingList(true);
      api.get("/return-entry")
         .then(res => {
            const list = Array.isArray(res.data) ? res.data : (res.data?.data ?? []);
            setEntries(list);
         })
         .catch(() => showError("Failed to load return entries."))
   ...
   function handleSave() {
      ...
      if (isEditing && editSnapshot.current) {
         ...
         const catUnchanged = CATEGORY_CONFIG.every(cat =>
            JSON.stringify(snap[cat.key]) === JSON.stringify(payload[cat.key])
         );
         if (snap.financialYear === payload.financialYear &&
             snap.returnDate    === payload.returnDate    &&
             snap.remarks       === payload.remarks       &&
             catUnchanged) {
      ...
      const req = isEditing
         ? api.put(`/return-entry/${editingId}`, payload)
         : api.post("/return-entry", payload);

      req.then(() => {
         showSuccess(isEditing ? "Return entry updated successfully." : "Return entry saved successfully.");
   ...
   function handleEdit(entry) {
      ...
      const categoryFields = Object.fromEntries(
         CATEGORY_CONFIG.map(cat => [
            cat.key,
            Object.fromEntries(cat.items.map(i => [i.key, entry[cat.key]?.[i.key] ?? ""]))
         ])
      );
      setForm({
   ...
   function handleDeleteConfirm() {
      ...
      api.delete(`/return-entry/${entryId}`)
         .then(() => {
            setConfirmOpen(false); setDeleteTarget(null);
            showSuccess("Return entry deleted successfully.");
      ...
      <div className="flex flex-col h-full overflow-hidden bg-slate-50">
         ...
         <div className="flex-none bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between flex-wrap gap-3">
            <div>
               ...
               <h1 className="text-xl font-bold text-slate-800 tracking-tight">Return Entries</h1>
               <p className="text-xs text-slate-400 mt-0.5">Items returned by dealers — reduces outstanding balance</p>
            </div>
            ...
            {PAGE_TABS.map(t => {
               ...
               return (
                  <button
                     ...
                     className={[
                        "px-6 py-2.5 text-sm font-semibold border-b-2 -mb-0.5 transition-colors cursor-pointer whitespace-nowrap",
                        isActive
                           ? isEditTab
                  ...
                  >
                     {isEditTab ? "Edit Return" : t.label}
                     {t.key === "entries" && entries.length > 0 && (
                        <span className={[
                           "ml-2 text-[10px] font-bold px-1.5 py-0.5 tabular-nums",
