...
//    const [s] = fy.split("-");
//    const full = parseInt(s) >= 50 ? `19${s}` : `20${s}`;
//    return `FY ${full}-${fy.split("-")[1]}`;
// }
...
//       return { valid: false, error: `Too large (${formatFileSize(file.size)}). Max 10 MB.` };
...
// export function formatFY(fy) {
//    // ✅ empty/null handle karo
//    if (!fy) return "FY —";
//    const parts = fy.split("-");
//    if (parts.length !== 2) return `FY ${fy}`;
